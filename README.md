...
#define POT_PIN      5
#define BUTTON1_PIN  12 // Nút Quạt
#define BUTTON2_PIN  10 // Nút Mode
#define BUTTON3_PIN  47 // Nút Đèn
#define PIR_PIN      18  
#define IN1_PIN      4  
#define IN2_PIN      2  
#define ENA_PIN      22  
...
// BIẾN XỬ LÝ DỘI PHÍM (CHỐNG NHẢY BẬC)
...
typedef struct { int16_t *buffer; uint8_t buf_ready; uint32_t buf_count; uint32_t n_samples; } inference_t;
...
void AITask(void * parameter) {
    if (!microphone_inference_start(EI_CLASSIFIER_RAW_SAMPLE_COUNT)) { vTaskDelete(NULL); }
    while(1) {
        if (!microphone_inference_record()) { vTaskDelay(10 / portTICK_PERIOD_MS); continue; }
        signal_t signal; signal.total_length = EI_CLASSIFIER_RAW_SAMPLE_COUNT; signal.get_data = &microphone_audio_signal_get_data;
            ...
            for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
                if (result.classification[ix].value > 0.5) { 
                    if (strcmp(result.classification[ix].label, "light_on") == 0) {
                        autoMode = false; ledLevel = 2; setLedPWM(ledLevel);
                    }
                    else if (strcmp(result.classification[ix].label, "light_off") == 0) {
                        autoMode = false; ledLevel = 0; setLedPWM(ledLevel);
                    }
                }
            }
        }
...
void setup() {
  ...
  if (!rtc.begin()) { Serial.println("Khong tim thay RTC!"); lcd.setCursor(0, 1); lcd.print("RTC Error!      "); }
  else { rtc.adjust(DateTime(2026, 5, 21, 17, 59, 0)); }
...
BLYNK_CONNECTED() { Blynk.syncAll(); isBlynkSynced = true; }

BLYNK_WRITE(V3) { ledLevel = param.asInt(); setLedPWM(ledLevel); autoMode = false; }
BLYNK_WRITE(V4) { fanLevel = param.asInt(); setFanPWM(fanLevel); autoMode = false; }

// ===============================================
// HÀM XỬ LÝ NÚT NHẤN MƯỢT MÀ
// ===============================================
void checkButtons() {
  int b1 = digitalRead(BUTTON1_PIN); // Nút Quạt
  int b2 = digitalRead(BUTTON2_PIN); // Nút Mode
  int b3 = digitalRead(BUTTON3_PIN); // Nút Đèn

  // BẤM NÚT MODE
  if (b2 == LOW && lastBtn2State == HIGH && (millis() - lastButton2 > 100)) {
    autoMode = !autoMode;
    if (!autoMode) {
      // Về Manual -> Trả Đèn + Quạt về OFF
      fanLevel = 0; ledLevel = 0; setFanPWM(0); setLedPWM(0);
      Serial.println("=> MODE: MANUAL (DA TAT HET TBI)");
    } else {
  ...
  // BẤM NÚT QUẠT
  if (b1 == LOW && lastBtn1State == HIGH && (millis() - lastButton1 > 100)) {
    if (autoMode) {
      autoMode = false; // Đang auto mà bấm nút -> văng ra Manual
      fanLevel = 0; ledLevel = 0; setFanPWM(0); setLedPWM(0);
      Serial.println("=> Ngan can AUTO -> MANUAL (DA TAT HET TBI)");
    } else {
  ...
  // BẤM NÚT ĐÈN
  if (b3 == LOW && lastBtn3State == HIGH && (millis() - lastButton3 > 100)) {
    if (autoMode) {
      autoMode = false; // Đang auto mà bấm nút -> văng ra Manual
      fanLevel = 0; ledLevel = 0; setFanPWM(0); setLedPWM(0);
      Serial.println("=> Ngan can AUTO -> MANUAL (DA TAT HET TBI)");
...
void loop() {
  ...
  // Đọc nút nhấn mượt mà
  checkButtons();

  // Chỉ chạy logic Auto nếu đang ở Auto Mode
  if (autoMode) autoControl();

  if (millis() - lastLCDUpdate > 500) { updateLCD(); lastLCDUpdate = millis(); }

  if (WiFi.status() == WL_CONNECTED && millis() - lastBlynkUpdate > 2000) { updateBlynk(); lastBlynkUpdate = millis(); }
...
void autoControl() {
  ...
  if (fanLevel != expectedFanAutoLevel) { fanLevel = expectedFanAutoLevel; setFanPWM(fanLevel); }

  // ĐÈN VÀ RADAR
  DateTime now = rtc.now();
  bool isNightTime = (now.hour() >= turnOnHour || now.hour() < turnOffHour);
  if (digitalRead(PIR_PIN) == HIGH) {
    if (!isPersonPresent) { firstDetectedTime = millis(); isPersonPresent = true; }
    lastMotionTime = millis();
    unsigned long stayDur = millis() - firstDetectedTime;
    if (stayDur > 30UL*60000UL) currentTimeout = 60UL*60000UL; 
    else if (stayDur > 10UL*60000UL) currentTimeout = 30UL*60000UL; 
    else if (stayDur > 5000UL) currentTimeout = 10UL*60000UL; 
  ...
  int expectedLedAutoLevel = (isNightTime || isTimerActive) ? 2 : 0; 
  if (ledLevel != expectedLedAutoLevel) { ledLevel = expectedLedAutoLevel; setLedPWM(ledLevel); }
}

void updateBlynk() { Blynk.virtualWrite(V0, currentTemp); Blynk.virtualWrite(V1, currentHum); Blynk.virtualWrite(V3, ledLevel); Blynk.virtualWrite(V4, fanLevel); }
...
void updateLCD() {
  ...
  lcd.setCursor(0, 1); lcd.print(nhietdoduocdieuchinh(), 1); lcd.print("C ");
...
// === CÁC HÀM XỬ LÝ I2S CHO AI GIỮ NGUYÊN BÊN DƯỚI NÀY ===
...
static void capture_samples(void* arg) { size_t bytes_read = (uint32_t)arg; while (record_status) { i2s_read((i2s_port_t)1, (void*)sampleBuffer, (uint32_t)arg, &bytes_read, 100); if (bytes_read > 0) { for (int x = 0; x < (uint32_t)arg/2; x++) sampleBuffer[x] = (int16_t)(sampleBuffer[x]) * 8; if (record_status) audio_inference_callback((uint32_t)arg); else break; } } vTaskDelete(NULL); }
static bool microphone_inference_start(uint32_t n_samples) { inference.buffer = (int16_t *)malloc(n_samples * sizeof(int16_t)); if(inference.buffer == NULL) return false; inference.buf_count = 0; inference.n_samples = n_samples; inference.buf_ready = 0; i2s_init(EI_CLASSIFIER_FREQUENCY); ei_sleep(100); record_status = true; xTaskCreatePinnedToCore(capture_samples, "CaptureSamples", 1024 * 32, (void*)sample_buffer_size, 10, NULL, 0); return true; }
static bool microphone_inference_record(void) { while (inference.buf_ready == 0) { vTaskDelay(10 / portTICK_PERIOD_MS); } inference.buf_ready = 0; return true; }
...
static int i2s_init(uint32_t sampling_rate) { i2s_config_t i2s_config = { .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX | I2S_MODE_TX), .sample_rate = sampling_rate, .bits_per_sample = (i2s_bits_per_sample_t)16, .channel_format = I2S_CHANNEL_FMT_ONLY_RIGHT, .communication_format = I2S_COMM_FORMAT_I2S, .intr_alloc_flags = 0, .dma_buf_count = 8, .dma_buf_len = 512, .use_apll = false, .tx_desc_auto_clear = false, .fixed_mclk = -1, }; i2s_pin_config_t pin_config = { .bck_io_num = I2S_SCK, .ws_io_num = I2S_WS, .data_out_num = -1, .data_in_num = I2S_SD, }; i2s_driver_install((i2s_port_t)1, &i2s_config, 0, NULL); i2s_set_pin((i2s_port_t)1, &pin_config); i2s_zero_dma_buffer((i2s_port_t)1); return 0; }
````
-------------------------------------------------------------------------
